...
def converted_txt_files():
    path="F:/programming projects/web scrapper/text_automated_files/"
    # iterating every pdf file in pdf folder and adding it to list
    for text_file in os.listdir(path):
        converted_text_files.append(text_file[:-4])
converted_txt_files()

all_pdf_file = []
def pdf_files():
    path_pdf = "F:/programming projects/web scrapper/pdf"
    for pdf_file in os.listdir(path_pdf):
        all_pdf_file.append(pdf_file[:-4])
pdf_files()

file_not_dwnld = []
def checking_not_down():
    for i in all_pdf_file:
        if i not in converted_text_files:
            file_not_dwnld.append(i+'.pdf')
checking_not_down()

print(len(file_not_dwnld))
...
pdf_string = reshape_list(file_not_dwnld[:-9],10,5)
